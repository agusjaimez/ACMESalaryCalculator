...
class Worker:
    def __init__(self,name: str, work: str) -> None:
        ...
        self.work = work.split(",")
    
    def getDaysAndHours(self, work: list[str]) -> list[list[str]]:
        r = []
        for day in work:
            #Separate day from hours
            matchedDay = re.split(r'([A-Z]{2})',day)
            matchedDay.remove('')
            #Add list containing 2 elements: day and range of hours worked
            r.append(matchedDay) 
        return r

    #This method calculates the salary of a day
    def calculateDailySalary(self, startHour: int, startMinutes: int, finishHour: int, finishMinutes: int, day: str):
        flagIs00 = False
        ...
        ranges = [(datetime.combine(date.today(),time(0,0)),datetime.combine(date.today(),time(9,0)),25,30),(datetime.combine(date.today(),time(9,1)),datetime.combine(date.today(),time(18,0)),15,20),(datetime.combine(date.today(),time(18,1)),datetime.combine(date.today(),time(23,59)),20,25)]
        dailySalary = 0
        ...
        timeWorked = datetime.combine(date.today(),time(int(hours),int(minutes*60)))
        ...
        for i in range(ranges.__len__()):
            ...
            if hourRange[0] <= entryHour <= hourRange[1] and hourRange[0] <= exitHour <= hourRange[1]:
                dailySalary = timeWorked.hour*hourlyRate + (timeWorked.minute/60)*hourlyRate
            #If the start hour matches a range but the finish hour exceeds it then this is used
                ...
                excedingHours = datetime.combine(date.today(),time(int(hours),int(minutes*60)))
                ...
                if excedingHours.hour> 8:
                    ...
                    extraExcedingHours = excedingHours - datetime.combine(date.today(),time(8,59))
                    hours,minutes = divmod(extraExcedingHours.seconds/60/60,1)
                    extraExcedingHours = datetime.combine(date.today(),time(int(hours),int(minutes)))
                    dailySalary = (timeWorked.hour + timeWorked.minute/60-excedingHours.hour-excedingHours.minute/60)*hourlyRate + (excedingHours.hour+excedingHours.minute/60-extraExcedingHours.hour-extraExcedingHours.minute/60) * ranges[i+1][paymentIndex] + (extraExcedingHours.hour+extraExcedingHours.minute/60) * ranges[i+2][paymentIndex]
                #In case hours only exceed the first range
                else:
                    dailySalary = (timeWorked.hour + timeWorked.minute/60-excedingHours.hour-excedingHours.minute/60)*hourlyRate + (excedingHours.hour+excedingHours.minute/60) * ranges[i+1][paymentIndex]
    ...
    def calculateSalary(self):
        ...
        daysAndHours = self.getDaysAndHours(self.work)
        salary = 0
        for day in daysAndHours:
            #Separate start and finish hour
            hours = day[1].split("-")
            #Separating minutes from hours
            startHour = hours[0].split(":")
            finishHour = hours[1].split(":")
            startMinutes = int(startHour[1])
            startHour = int(startHour[0]) #Redefining variable so as not to create a new one
            finishMinutes = int(finishHour[1])
            finishHour = int (finishHour[0]) #Redefining variable so as not to create a new one
            salary += self.calculateDailySalary(startHour, startMinutes, finishHour, finishMinutes, day[0])
        return salary


    def __str__(self) -> str:
        return "The amount to pay "+self.name+" is: "+str(self.calculateSalary())+"USD"